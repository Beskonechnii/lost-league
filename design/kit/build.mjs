...
// Разметка листов лежит в parts/<Имя>.html, хвост CSS одного листа — в parts/<Имя>.css.
// Лист со своим стилем (общий STYLE не наследует) кладёт его в parts/<Имя>.solo.css.
// Правим parts/, потом node build.mjs — .dc.html целиком вывод, руками их не трогаем.
const p = (f) => new URL("./parts/" + f, import.meta.url);
const read = (f) => (existsSync(p(f)) ? readFileSync(p(f), "utf8") : null);

const names = readdirSync(new URL("./parts/", import.meta.url))
  .filter((f) => f.endsWith(".html"))
  .map((f) => f.replace(/\.html$/, ""))
  .sort();

for (const name of names) {
  const solo = read(`${name}.solo.css`);
  const css = solo ?? STYLE + (read(`${name}.css`) ?? "");
  writeFileSync(
    new URL(`./${name}.dc.html`, import.meta.url),
    `<!doctype html>
<html>
<head>
  <meta charset="utf-8">
...
console.log("собрано артбордов:", names.length);
