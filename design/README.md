...
**Осторожно: генератор отстал.** В канвасе 41 артборд, `build.mjs` умеет 15 — компонентные листы
(Alert, Toggle, TextField, …) заведены прямо в канвасе. Их `.dc.html` лежат в репозитории, но
`node build.mjs` их не перезапишет и не удалит; публиковать кит нужно, засеивая **все** файлы
папки, а не только вывод генератора. Строка в `BACKLOG.md`.
