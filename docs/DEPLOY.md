...
> Пошаговый прогон руками, с проверками на каждом шаге и диагностикой —
> **`docs/DEPLOY-MANUAL.md`**. Здесь то же самое, плюс вариант B (голый Node) и вариант C.
...
| `OWNER_EMAIL` | **да** | почта владельца лиги: кто войдёт под ней, получает роль `owner` и раздаёт права остальным. Пароля-админки в проекте нет — доступ даёт роль |
| `AUTH_SECRET` | **да** | подпись сессии, `openssl rand -hex 32`. Пусто = выводится из `GOOGLE_CLIENT_SECRET`, а при входе по паролю без Google сессия не подписывается |
...
Контейнер намеренно слушает только `127.0.0.1:3001` — без этого админка торчала бы
...
git clone https://github.com/Beskonechnii/lost-league.git
cd LOST/web
cp .env.example .env && nano .env      # заполнить OWNER_EMAIL, AUTH_SECRET, BASE_URL
