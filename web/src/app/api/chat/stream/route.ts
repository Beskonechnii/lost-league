...
export async function GET(req: Request) {
  // Канал шире лички: его держит ЛЮБОЙ вошедший аккаунт — админ комнаты без профиля в ростере
  // (ТЗ 22б §8) и заявитель, которому лига пишет о его заявке. Присутствие при этом не меняется —
  // оно считается по playerId.
  const me = await currentLiveMe();
  if (!me) return bad("Живой канал — для вошедших", 401);
