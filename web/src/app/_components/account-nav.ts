...
export type AccountNav = {
  ...
  /** Живой канал шире лички: его открывает любой вошедший аккаунт, в том числе без профиля
...
export async function accountNav(account: Account | null): Promise<AccountNav> {
  ...
  // ЛЮБОМУ вошедшему аккаунту (`tellAccount`) — оператору без карточки игрока, которому приходят
  // строки очередей (ТЗ 28 DESIGN §4), и заявителю, которому лига отвечает по его заявке ещё до
  // одобрения. По `chatIdentity` колокольчика у них не было вовсе, и уведомление уходило в никуда.
