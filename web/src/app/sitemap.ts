 ...
 *   `/tournaments/<слаг>` без дивизиона, `/mixcup/*` — ТЗ 34/37) — карта должна называть
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const [tournaments, teams, players, series] = await Promise.all([
    prisma.tournament.findMany({
      ...
      select: { slug: true, kind: true, status: true, divisions: { select: { slug: true } } },
    }),
    prisma.team.findMany({ where: { archivedAt: null }, select: { id: true } }),
    // Непроверенный (запись до апрува, ТЗ 34) в карту не идёт — у него нет своей публичной
  ...
  for (const t of tournaments) {
    // Турнир индивидуального формата (ТЗ 37) адресуется страницей записи: ни вкладки «О турнире»,
    // ни дивизионов у него нет, а /tournaments/<слаг> для него — редирект.
    if (t.kind !== "season") {
      entries.push({ url: url(`/join/${t.slug}`), priority: t.status === "registration" ? 0.8 : 0.5 });
      continue;
    }
