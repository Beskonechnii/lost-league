 ...
 * номер переезжает в сам слот, а список 1..20 распадается на две группы по действию. Ради этого
 * этап и делался: «что взяла эта команда» читается одним взглядом, а пять пиков подряд одного
 * размера — и есть картинка референса. Номера ходов (1..20) никуда не деваются, они в слотах.
 *
  ...
  return (
    ...
    >
      <Panel>
        ...
        {/* Ширина ряда закреплена: 224px — это внутренняя ширина колонки на xl, то есть плитка
            ровно 40px. В стеке ниже xl колонка растягивается на всю страницу, и без потолка
            пять банов раздувались бы вдвое крупнее пиков. */}
        <ol className="grid max-w-[224px] grid-cols-5 gap-1.5">
          {bans.map(({ i }) => {
            const { n, hero, current } = slotOf(i);
            return (
...
/** Слот бана: плитка ~40×23 в ряду из пяти, номер подписью под ней, имя — зачёркнутым.
...
function BanSlot({ n, hero, current }: { n: number; hero: HeroRef | undefined; current: boolean }) {
  return (
    <div className="min-w-0">
      ...
      {/* Строка держится всегда: иначе ряд из пяти плиток скачет по высоте по мере банов. */}
