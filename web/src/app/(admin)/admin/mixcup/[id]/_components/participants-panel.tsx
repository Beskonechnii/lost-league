...
export type Registrant = {
  ...
  nickname: string;
  photo: string | null;
  /** Не прошёл модерацию (Mix Cup до апрува, ТЗ 34) — подпись «новичок, не проверен» и в
   *  этом списке, и в пуле драфта: оператор обязан отличать их с первого взгляда. */
  verified: boolean;
};

/**
 * Список записавшихся (Scope п.5/6 ТЗ 34) — и разрез «пришли с Mix Cup» списком (непроверенные
 * подписаны прямо в строке), и фильтром (таб «Новички»), и кнопка «взять участников в драфт»,
 * которая заполняет фазу отбора ровно этим списком, а не отмечается вручную по ростеру.
 */
export function ParticipantsPanel({ eventId, items }: { eventId: number; items: Registrant[] }) {
  ...
  async function takeToDraft() {
    setBusy(true);
    try {
      const started = await fetch(`/api/mixcup/${eventId}/draft`, { method: "POST" });
      if (!started.ok) throw new Error(await started.text());
      const filled = await fetch(`/api/mixcup/${eventId}/participants`, { method: "POST" });
  ...
  return (
    <Panel
      title="Участники"
      hint="Записались на Mix Cup — оператор берёт их в драфт одной кнопкой, а не отмечает руками по ростеру."
      aside={
    ...
    >
      {items.length === 0 ? (
        <EmptyState icon="flame" title="Пока никто не записался">
          Ссылка на регистрацию — со страницы события: /mixcup/&lt;слаг&gt;.
        </EmptyState>
      ) : (
