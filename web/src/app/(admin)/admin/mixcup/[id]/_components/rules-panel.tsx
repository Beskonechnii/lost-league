...
/**
 * Тумблеры правил события: «Украсть» и «Закрепить», вкл/выкл — ровно два, чисел (сколько краж
 * на команду) в этом этапе нет (BACKLOG «Настройки правил Mix Cup числом»).
 *
 * До старта драфта активны и пишут в событие (а если живая сессия уже есть в roster/config —
 * сервер синхронно правит и её DraftState, см. `PATCH /api/mixcup/[id]`). С фазы draft —
 * `disabled` с подсказкой, а не спрятаны: оператор должен видеть, что было включено.
 */
export function RulesPanel({
  ...
  async function patch(patch: { stealEnabled?: boolean; lockEnabled?: boolean }) {
    setBusy(true);
    try {
      const res = await fetch(`/api/mixcup/${eventId}`, {
        method: "PATCH",
  ...
  return (
    <Panel title="Правила" hint="Тумблеры действуют только для этого Mix Cup — константы UNDERBEER не трогают.">
