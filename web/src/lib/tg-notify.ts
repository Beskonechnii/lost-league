...
/**
 * Ссылка на смену забытого пароля (ТЗ 02). Единственный канал доставки: писем лига не шлёт.
 *
 * Отправку так же глотаем молча — и по той же причине, что остальные уведомления здесь, и по
 * дополнительной: форма запроса обязана отвечать одинаково всем, а «не доставлено» на экране
 * означало бы «такой аккаунт есть, но бот его не достал».
 */
export async function notifyPasswordReset(accountId: number, url: string, ttlMin: number): Promise<void> {
  await tellAccount(
    accountId,
    `Кто-то запросил смену пароля на сайте лиги. Если это вы — вот ссылка, она одноразовая и живёт ${ttlMin} минут:\n\n${url}\n\n` +
      "Если это не вы — просто не открывайте её: пароль останется прежним, а ссылка сгорит сама.",
  );
}
