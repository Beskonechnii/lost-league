...
/**
 * Ответы на предложение соперника. Приезжают без диалога — клавиатуру ставит само уведомление, и
 * нажимают её из любого места, где человек стоял. С Э7 эти две подписи объявлены **точкой входа**
 * флоу `answer` (`bot-flow/default-flow.ts`): что случится по нажатию, описано графом, а не кодом.
 */
