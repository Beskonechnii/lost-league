...
export const FLOW_ACTIONS: Record<string, FlowAction> = {
  ...
  // Про откат `QUIZ_ROSTER` (`tg-menu.ts`): граф о нём не знает и всегда ведёт на сайт. Понадобится
  // вернуть квиз состава — кнопке «Подать заявку» в графе снимают переход, и её разберёт старый путь.
  заявка: {
    label: "Подать заявку",
