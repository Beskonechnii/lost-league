...
//
// **Что граф ведёт с Э4: первый уровень и все справки.** Профиль, код входа, турниры, свой состав и
// команды соперников — это ноды-действия поверх того же кода, что зовёт рукописный путь
// (`actions.ts`). За старым обработчиком остаётся то, что пишет в базу диалогом: регистрация,
// правка профиля, заказ встречи, анкеты и квиз заявки. У их кнопок здесь `next: null` — «граф
// показал, дальше разбирается `tg-quiz.ts`»; нодами `subflow` они станут на Э5.
//
 ...
 * лига не знает, анкеты — когда открытая анкета есть; у обеих перехода нет: их ведёт старый путь.
 */
const mainButtons = (register: FlowButton["when"]): FlowButton[] => [
  { label: REGISTER_BUTTON, next: null, when: register, row: 8 },
  { label: MENU.profile, next: "profile-do", row: 9 },
  { label: MENU.tournaments, next: "tour-list", row: 9 },
  { label: FORMS_BUTTON, next: null, when: { left: "ctx.есть_анкеты", op: "=", right: "да" }, row: 10 },
];
  ...
  nodes: [
...

    {
      ...
      buttons: [
        ...
        { label: EDIT_BUTTON, next: null, when: { left: "vars.правка", op: "=", right: "да" }, row: 0 },
...

    {
      ...
      buttons: [
        ...
        // нельзя. Сам заказ ведёт `tg-meetings.ts` (Э5).
        { label: MEETING_BUTTON, next: null, when: { left: "vars.встреча", op: "=", right: "да" }, row: 2 },
