...
//
// **Почему исходящее не в графе** (решено на Э7, блок в `DECISIONS.md`). Нод-триггеров по
// расписанию у графа нет: у ноды-триггера нет входящего сообщения, нет чата-адресата и нет
// разговора, в котором она стоит, — то есть нет ничего, из чего граф состоит. Стык между рассылкой
// и диалогом сделан со стороны **входа**: кнопка, которую ставит уведомление, объявлена точкой
// входа своего флоу (`bot-flow/default-flow.ts` → `answer`), и что случится по нажатию, правится
// в редакторе, а не здесь.
