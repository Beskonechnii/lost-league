// Настройки флоу бота: тайминги напоминаний и тексты уведомлений о встречах. Уклад тот же, что у
// графа диалога (`bot-flow/default-flow.ts`): реестр с дефолтами живёт в коде, в таблице
// `BotSetting` лежат только отличия оператора. Сброшенная база означает бота с обычными
// настройками, а не бота без таймингов.
//
// **Почему это не уехало в граф вместе с вопросами (Э6).** У настройки нет входящего сообщения и
// потому нет ноды: `remind_before` и `no_time_digest_at` — про исходящий поток (`tg-schedule.ts`),
// который бот шлёт сам. Диалог живёт нодами, расписание — здесь.
//
...
/** Реестр настроек в порядке экрана. Им же рисуется страница /admin/bot. */
